...
def test_get_positive_int_env_uses_default_when_missing(monkeypatch):
    ...
    assert get_positive_int_env("RETRIEVER_K", DEFAULT_RETRIEVER_K) == DEFAULT_RETRIEVER_K
...
def test_settings_reads_retriever_k_from_environment(monkeypatch):
    monkeypatch.setenv("RETRIEVER_K", str(TEST_RETRIEVER_K))

    assert Settings().retriever_k == TEST_RETRIEVER_K
